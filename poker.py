...
preFlop_text = font2.render("PreFlop Round", True, GOLD)
flop_text = font2.render("Flop Round", True, GOLD)
turn_text = font2.render("Turn Round", True, GOLD)
river_text = font2.render("River Round", True, GOLD)
...
mainmenu_button = Button(1300, 420, "EXIT", font, WHITE, BLACK, 200, 100)
next_button = Button(1300, 540, "NEXT", font, WHITE, BLACK, 200, 100)

call_button = Button(1300, 540, "NEXT", font, WHITE, BLACK, 200, 100)
raise_button = Button(1300, 540, "NEXT", font, WHITE, BLACK, 200, 100)
fold_button = Button(1300, 540, "NEXT", font, WHITE, BLACK, 200, 100)
...
def main_menu():
    while True:
        ...
        # screen.blit(judul_text,(680,150))
...
def main():
    ...
    while running:
        ...
        if start_button.draw(screen):
            ...
            screen.blit(preFlop_text,(1000,50))
            pre_flop()
            pygame.display.flip() 
        ...
        if next_button.draw(screen):
            ...
            if round == 0:
                print("Flop")
                screen.blit(flop_text,(1000,50))
                flop()
                round = 1
                is_flop = True
            elif round == 1:
                print("Turn")
                screen.blit(turn_text,(1000,50))
                turn()
                round = 2
                is_turn = True
            elif round == 2:
                print("River")
                screen.blit(river_text,(1000,50))
                river()
                round = 3
                is_river = True
        elif mainmenu_button.draw(screen):
...
player1_images = []  # Gambar untuk player
player2_images = []

def detect_card(screen):
    ...
    while True:
        ...
        resized_frame = cv2.cvtColor(resized_frame, cv2.COLOR_BGR2RGB)  # Pastikan diubah ke RGB
        frame_surface = pygame.image.frombuffer(resized_frame.tobytes(), (new_width, new_height), 'RGB')
        screen.blit(frame_surface, (150, 250))
        pygame.display.flip()
                ...
                if key == pygame.K_s:  
                    if predictions:
                        ...
                        try:
                            ...
                            if len(dealer_images) == 5 and len(player1_images) == 2 :
                                deal_cards_for_player2()
                                cap.release()
...
def deal_cards_for_player2():
    remaining_cards = [card for card in all_cards if card not in dealer_deck and card not in player1_deck]
    random.shuffle(remaining_cards)
    player2_cards = remaining_cards[:2] #ambil 2 kartu
    # save gambar kartu player2
    for card in player2_cards:
        try:
            image_path = os.path.join('images', f"{card}.jpg")
            prediction_image = pygame.image.load(image_path)
            player2_images.append(prediction_image)
            player2_deck.append(card)
        except FileNotFoundError:
            print(f"Gambar untuk '{card}' tidak ditemukan.")
...
def evaluate_deck(deck):
    ...
    most_common = value_counts.most_common()  # [(nilai, jumlah), ...]
...
def river():
    ...
    print(f"Winner: {best_deck_info['winner']}")
    print(f"Winning Combination: {best_deck_info['winning_combination_type']}")
    print(f"Player 1 Best Hand: {best_deck_info['player1']['best_combination_type']} ({best_deck_info['player1']['best_value']})")
    print(f"Player 2 Best Hand: {best_deck_info['player2']['best_combination_type']} ({best_deck_info['player2']['best_value']})")
